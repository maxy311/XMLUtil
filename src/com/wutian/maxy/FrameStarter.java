...
public class FrameStarter implements ButtonClickInterface {
	...
	/*
	 * originpath:resPath targetPaht:翻译路径
	 * 
	 */
	@Override
	...
	public void compareFile(String originPath, String targetPath, String savePath, boolean isCompareAr) {
		...
		for (File file : originFile.listFiles()) {
			if (!file.getName().contains("string") || file.getName().contains("filter")
					|| file.getName().contains("account") || file.getName().contains("product_setting"))
				continue;
	...
	public void standardFile(String srcPath) {
		...
		for (File file : enDir.listFiles()) {
			...
			if (name.contains("dimen") || name.contains("style") || name.contains("attr") || name.contains("color")
					|| name.contains("id"))
				continue;
